$(function () {
    ...
    $('#login_form').submit(function() {
        ...
        if (temp_name == '') {
            ...
            socket.emit('new user', $('#username').val(), $('#chatroomName').val(), function(data) {
                if (data) {
                    $('#login_section').hide();
                    $('#chat_section').show();
                    chatroomName = $('#chatroomName').val();
                    $('#chatroomHeader').text('Chatroom ' + chatroomName);
                    if (chatroomName == "")
                        chatroomName = "__default";
                } else {
                    $('#username').val('');
    ...
    // Get user list
    socket.on('get users', function (users, crName) {
        if (crName == chatroomName) {
    ...
    socket.on('get messages', function (messages, crName) {
        if (crName == chatroomName) {
    ...
    socket.on('chat message', function (data, crName) {
        if (crName == chatroomName) {
    ...
    socket.on('online message', function (data, crName) {
        if (crName == chatroomName) {
    ...
    socket.on('offline message', function (data, crName) {
        if (crName == chatroomName) {
            $('#messages').append('<div class="list text-center">\
